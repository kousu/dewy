...
class VideoReader(object):
    ...
    # TODO
    width = vidproperty(cv2.CAP_PROP_FRAME_WIDTH, int, "Width of the frames in the video stream")
    height = vidproperty(cv2.CAP_PROP_FRAME_HEIGHT, int, "Height of the frames in the video stream")
    fps = vidproperty(cv2.CAP_PROP_FPS, float, "Frame rate")
    codec = vidproperty(cv2.CAP_PROP_FOURCC, doc="4-character code of codec")
    format = vidproperty(cv2.CAP_PROP_FORMAT, doc="Format of the Mat objects returned by next()")
    _msec = vidproperty(cv2.CAP_PROP_POS_MSEC, float, "Current position of the video file in milliseconds or video capture timestamp")
    _count = vidproperty(cv2.CAP_PROP_FRAME_COUNT, int, "Number of frames in the video file.")
    ...
    _frame = vidproperty(cv2.CAP_PROP_POS_FRAMES, int, doc="1-based index of the current frame")
